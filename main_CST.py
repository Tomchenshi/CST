...
def main():
    ...
    train_parser.add_argument("--model_title", type=str, default="CST", help="model_title, default set to model_title")
...
def train(args):
    ...
    train_path = './datasets/'+args.dataset_name+'_x'+str(args.n_scale)+'/trains/'
    result_path = './results/' + args.dataset_name + '_x' + str(args.n_scale)+'/'
    test_data_dir = './datasets/'+args.dataset_name+'_x'+str(args.n_scale)+'/'+args.dataset_name+'_test.mat'
    ...
    net = CST(inp_channels=colors, dim=args.n_feats, depths=[4,4,4,4], num_heads=[6,6,6,6],mlp_ratio=2, scale=args.n_scale)
    # print(net)
    ...
    print('===> Start testing')
    model_name = './checkpoints/' + traintime +'/' + "_" + args.model_title + "_ckpt_epoch_" + str(best_epoch) + ".pth"
    with torch.no_grad():
        test_number = 0
        epoch_meter = meter.AverageValueMeter()
        epoch_meter.reset()
        # loading model
        net = CST(inp_channels=colors, dim=args.n_feats, depths=[4, 4, 4, 4],
...
def test(args):
    ...
    #model_name = './checkpoints/' +'/'+args.dataset_name +'_'+ model_title + "_ckpt_epoch_" + str() + ".pth"
